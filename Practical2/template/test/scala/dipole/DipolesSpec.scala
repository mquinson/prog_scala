...
class DipolesSpec extends FlatSpec with Matchers with Inside with Inspectors {
  ...
  it should "pass this second test on impedance()" in {
    ...
    assertResult(Complex(0.2079131844185524, -4.55500719534011)) {
      (dip.impedance(omega))
    }
  }
  it should "have a working toString()" in {
	  val dip = Parallel( Inductor(5e-2), Capacitor(9e-4))
    assertResult("Parallel(Inductor(0.05), Capacitor(9.0E-4))") {
      (dip.toString())
    }
  }
  /* Nary circuits */
  "Nary serie circuit" should "have a working impedance()" in {
    ...
    assertResult(Complex(100,59.2952)) {
      dip.impedance(omega)
    }
  }
  ...
  it should "have a working toString()" in {
    val dip = Nparallel(Resistor(1e2) :: Capacitor(9e-4) :: Inductor(2e-1) :: Nil)
    assertResult("Parallel(Resistor(100.0), Capacitor(9.0E-4), Inductor(0.2))") {
      (dip.toString())
    }
  }
  /* Instances */
  "Stock instances" should "have a well defined dip1" in {
    assertResult("Parallel(Resistor(100.0), Serie(Inductor(5.0E-5), Resistor(12000.0)), Capacitor(9.000000000000001E-9))") {
      (Instances.dip1.toString())
    }
    //assertResult( Complex(99.17354592177011, -0.02780791066116556)) {
    assertResult( Complex(0.01008333333333319, 2.8273309166666674E-6)) {
      (Instances.dip1.impedance(omega))
    }
  }
  it should "have a well defined dip2" in {
    assertResult("Serie(Resistor(100.0), Parallel(Serie(Parallel(Serie(Resistor(1000.0), Inductor(5.0E-5)), Capacitor(0.009000000000000001), Serie(Capacitor(9.000000000000001E-4), Capacitor(1.0E-5))), Resistor(330.0)), Serie(Resistor(1000.0), Inductor(0.2))), Capacitor(1.0E-6))") {
      (Instances.dip2.toString())
    }
    assertResult( Complex(348.25678835078304, -3177.629770728981)) {
