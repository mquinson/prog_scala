...
\begin{columns}[t]
  \begin{column}{.5\textwidth}
    \begin{block}{Why? When?}
      ...
      \item Select which class to instanciate
      \item Cache objects
      \end{itemize}
    \end{block}
  \end{column}
...
\begin{frame}[t]{Last one, just for fun: dhyang IOCCC'00}
  ...
  \begin{columns}
    \begin{column}{.23\linewidth}\vspace{-\baselineskip}
      \begin{block}{Source code}\medskip
        \includegraphics[width=\linewidth]{img/dhyang1.png}        
      \end{block}
    \end{column}

    \begin{column}{.29\linewidth}\vspace{-\baselineskip}
  ...
  \visible<5->{This is a Quine: \url{https://en.wikipedia.org/wiki/Quine_(computing)}}
#+END_LaTeX
