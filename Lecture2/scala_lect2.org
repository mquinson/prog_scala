...
# non #+LATEX_HEADER: \setlength\partopsep{-1cm}
# non #+LATEX_HEADER: \setlength\topsep{-1cm}
# non #+LATEX_HEADER: \xpretocmd{\inputminted}{\par\vspace{-1cm}}{}{}
# non #+LATEX_HEADER: \xapptocmd{\inputminted}{\par\vspace{-1em}}{}{}
...
- Flexible ~import~: even within a function, or import methods as
  singletons
- Class files must be in the right relative subdirectory:\par \scriptsize ~bobsrocket/navigation/Navigator.class~\normalsize

*** Exceptions
:PROPERTIES:
:BEAMER_opt: fragile
:END:
**** \alert{Purpose}: Mechanism to deal with unexpected failures, \normalsize eg system errors
- \structure{Throw}n exception fly up in the call stack until it gets
  catched
...
\begin{columns}
  ...
  \begin{column}{.44\linewidth}
    \begin{itemize}
    \item Execution flow gets changed
    \item Breaks in the \texttt{try} block at first exception
    \item Branches to \texttt{catch} case (or out)
    \item Always runs the \texttt{finally} block
    \item Only catch the exceptions that you can deal with
