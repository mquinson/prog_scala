...
- C++ lets you inherit of several classes, but gets utterly complex if
  you do
  - \structure{Diamond Problem:} A inherits of B and C, which both
    inherit of D
  - Both B and C override the same method of D. What should A execute?
- Scala/Java: extend at most one class \small(simplicity at price of missed
  factorizations)\normalsize
\pause\medskip
...
- \structure{Limitation:} Constructor parameters cannot be objects
- \structure{Diamond Problem Mitigation:} \pause
  - Right-most Depth-first trait wins -- other method candidates are
    ignored
  - Won't work for constructors: calls cannot be eliminated
  - But thanks to the limitation above, traits have no dynamic
    constructor
