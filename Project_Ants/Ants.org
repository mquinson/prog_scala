...
on the long term. Download ~sbt~ from its [[http://www.scala-sbt.org/download.html][webpage]], unpack it somewhere
(e.g. under ~/opt/sbt~) and add the ~bin~ directory to your PATH.
...
source ~/.bashrc
# Launch the tool. It will download some missing parts 
sbt
#+END_SRC
#+LaTeX: \normalsize
